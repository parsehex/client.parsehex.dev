<script setup lang="ts">
  ...
  const deleteProject = async (id: string) => {
    if (!confirm('Are you sure you want to delete this project? This will also delete all updates and comments.')) return
    try {
      ...
      <UTable :columns="columns" :rows="projects" :loading="isLoading">
        ...
        <template #actions-data="{ row }">
          <div class="flex items-center gap-1">
            ...
            <UButton size="xs" color="red" variant="ghost" icon="i-lucide-trash" @click="deleteProject(row.id)" />
          </div>
        </template>
      </UTable>
    </UCard>
